...
[Authorize]
[ApiController]
[Route("api/catalog/shared")]
public class SharedCatalogController : ControllerBase
{
    private readonly ICatalogRepository _catalogRepository;

    public SharedCatalogController(ICatalogRepository catalogRepository)
    {
        _catalogRepository = catalogRepository;
    }

    [HttpGet]
    public async Task<IActionResult> GetSharedCatalogTypes(
        [FromQuery] string? keyword = null,
        [FromQuery] int? status = null)
    {
        var result = await _catalogRepository.GetCatalogTypesFilteredAsync(false, keyword, status);
        return Ok(result);
    }
    ...
    {
        var result = await _catalogRepository.GetCatalogTypeByIdFilteredAsync(id, false);
