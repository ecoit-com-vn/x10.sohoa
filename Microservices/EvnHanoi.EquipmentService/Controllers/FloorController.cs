...
[Authorize]
[ApiController]
[Route("api/catalog/floor")]
public class FloorController : ControllerBase
{
    private readonly ICatalogRepository _catalogRepository;
    private const string CatalogTypeCode = "TANG";

    public FloorController(ICatalogRepository catalogRepository)
    {
    ...
    public async Task<IActionResult> Create([FromBody] Catalog catalog)
    {
        ...
        if (existing != null)
            return BadRequest(new { message = $"Mã tầng hồ sơ '{catalog.Code}' đã tồn tại trong nhóm này." });
    ...
    public async Task<IActionResult> Update(long id, [FromBody] Catalog catalog)
    {
        ...
        if (existing != null && existing.Id != id)
            return BadRequest(new { message = $"Mã tầng hồ sơ '{catalog.Code}' đã được sử dụng bởi bản ghi khác." });
    ...
    public async Task<IActionResult> Delete(long id)
    {
        ...
        if (await _catalogRepository.HasChildrenAsync(id))
            return BadRequest(new { message = "Không thể xóa tầng hồ sơ này vì đang có các mục con tham chiếu tới." });
    ...
    public async Task<IActionResult> Lock(long id)
    {
        ...
        var success = await _catalogRepository.UpdateAsync(catalog);
        if (!success) return StatusCode(500, new { message = "Không thể cập nhật trạng thái tầng hồ sơ." });

        return Ok(new { message = "Đã khóa tầng hồ sơ thành công.", status = 0 });
    }
    ...
    public async Task<IActionResult> Unlock(long id)
    {
        ...
        return Ok(new { message = "Đã mở khóa tầng hồ sơ thành công.", status = 1 });
    }
