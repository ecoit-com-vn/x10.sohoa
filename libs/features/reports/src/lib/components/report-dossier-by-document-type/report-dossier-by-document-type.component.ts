...
import {
  ...
  DossierByDocumentTypeFilter,
  DossierByDocumentTypeChartStat
} from '../../data-access/report-dossier-by-document-type.service';
import { UnitLookupItem } from '../../data-access/report-dossier-by-year.service';
import {
  REPORT_STATISTICS_DOCUMENT_LIST_CONFIGS,
  REPORT_STATISTICS_DOCUMENT_TYPE_GRID_CONFIGS
...
import { ReportStatisticsDocumentListComponent } from '../report-statistics-document-list/report-statistics-document-list.component';
import { ReportStatisticsDocumentTypeGridComponent } from '../report-statistics-document-type-grid/report-statistics-document-type-grid.component';
...
@Component({
  selector: 'app-report-dossier-by-document-type',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
    ...
    ReportStatisticsDocumentListComponent,
    ReportStatisticsDocumentTypeGridComponent
  ],
  providers: [MessageService],
  templateUrl: './report-dossier-by-document-type.component.html',
  styleUrls: [
    '../report-dossier-by-year/report-dossier-by-year.component.scss',
    './report-dossier-by-document-type.component.scss'
  ]
})
export class ReportDossierByDocumentTypeComponent implements OnInit, AfterViewInit {
  private reportService = inject(ReportDossierByDocumentTypeService);
  private messageService = inject(MessageService);
  private authService = inject(AuthService);
  private router = inject(Router);

  @ViewChild(ReportStatisticsDocumentListComponent)
  documentList?: ReportStatisticsDocumentListComponent;

  @ViewChild(ReportStatisticsDocumentTypeGridComponent)
  documentTypeGrid?: ReportStatisticsDocumentTypeGridComponent;
  ...
  readonly documentListConfig = REPORT_STATISTICS_DOCUMENT_LIST_CONFIGS.DOSSIER_BY_DOCUMENT_TYPE;
  readonly documentTypeGridConfig = REPORT_STATISTICS_DOCUMENT_TYPE_GRID_CONFIGS.DOSSIER_BY_DOCUMENT_TYPE;
  filterVersion = signal(0);

  reportFilter = computed(() => ({
    unitId: this.selectedUnitId(),
    documentTypeIds: this.selectedDocumentTypeIds().filter((id) => id?.trim()),
    ...yearToFilterParam(this.selectedYear())
  }));

  units = signal<UnitLookupItem[]>([]);
  documentTypeOptions = signal<DocumentTypeOption[]>([]);
  years = signal<number[]>([]);
  ...
  selectedDocumentTypeIds = signal<string[]>([]);
  ...
  chartStats = signal<DossierByDocumentTypeChartStat[]>([]);
  ...
  private loadSecondaryLookups(): void {
    this.reportService.getDocumentTypesLookup().subscribe({
      next: (types) => {
        const options: DocumentTypeOption[] = (types || []).map((t: DocumentTypeLookupItem) => ({
          id: String(t.id),
          name: t.code ? `${t.code} — ${t.name}` : t.name
        }));
        this.documentTypeOptions.set(options);
      },
      error: (err) => console.error('Lỗi tải loại văn bản:', err)
    });
  ...
  loadStatsData(): void {
    const filter: DossierByDocumentTypeFilter = {
      unitId: this.selectedUnitId(),
  ...
  exportExcel(): void {
    if (this.activeTab() !== 'list') {
      this.messageService.add({
        severity: 'info',
        summary: 'Thông báo',
        detail: 'Xuất Excel áp dụng cho tab Danh sách tài liệu.'
      });
      return;
    }
    ...
    this.reportService.exportExcel(filter)
      ...
      .subscribe({
        next: (blob) => {
          ...
          a.href = url;
          a.download = `DanhSachTaiLieu_LoaiVanBan_${buildExportYearSuffix(this.selectedYear())}_${new Date().getTime()}.xlsx`;
          a.click();
          window.URL.revokeObjectURL(url);
          this.messageService.add({ severity: 'success', summary: 'Thành công', detail: 'Đã xuất danh sách tài liệu ra Excel.' });
        },
  ...
  getTypeLabel(stat: DossierByDocumentTypeChartStat): string {
    const name = stat.documentTypeName?.trim();
    if (name) return name;
    const code = stat.documentTypeCode?.trim();
    return code || '—';
  }
