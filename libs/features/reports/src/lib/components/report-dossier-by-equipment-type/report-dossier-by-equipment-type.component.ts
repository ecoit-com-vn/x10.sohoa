...
import {
  ...
  EquipmentTypeLookupItem,
  DossierByEquipmentTypeFilter,
  DossierByEquipmentTypeChartStat
} from '../../data-access/report-dossier-by-equipment-type.service';
...
@Component({
  selector: 'app-report-dossier-by-equipment-type',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
    ...
    ReportStatisticsEquipmentTypeGridComponent
  ],
  providers: [MessageService],
  templateUrl: './report-dossier-by-equipment-type.component.html',
  styleUrls: [
    '../report-dossier-by-year/report-dossier-by-year.component.scss',
    './report-dossier-by-equipment-type.component.scss'
  ]
})
export class ReportDossierByEquipmentTypeComponent implements OnInit, AfterViewInit {
  private reportService = inject(ReportDossierByEquipmentTypeService);
  ...
  @ViewChild(ReportStatisticsEquipmentTypeGridComponent)
  equipmentTypeGrid?: ReportStatisticsEquipmentTypeGridComponent;
  ...
  readonly dossierListConfig = REPORT_STATISTICS_DOSSIER_LIST_CONFIGS.DOSSIER_BY_EQUIPMENT_TYPE;
  readonly equipmentTypeGridConfig = REPORT_STATISTICS_EQUIPMENT_TYPE_GRID_CONFIGS.DOSSIER_BY_EQUIPMENT_TYPE;
  ...
  selectedEquipmentTypeIds = signal<string[]>([]);
  ...
  chartStats = signal<DossierByEquipmentTypeChartStat[]>([]);
  ...
  private loadSecondaryLookups(): void {
    ...
    this.reportService.getEquipmentTypesLookup().subscribe({
      next: (types) => {
        const options: EquipmentTypeOption[] = (types || []).map((t: EquipmentTypeLookupItem) => ({
          id: String(t.id),
          name: t.name
        }));
        this.equipmentTypeOptions.set(options);
      },
  ...
  onFilter(): void {
    ...
    this.equipmentTypeGrid?.resetPagination();
    this.loadStatsData();
  }
  ...
  loadStatsData(): void {
    const filter: DossierByEquipmentTypeFilter = {
      unitId: this.selectedUnitId(),
      objectType: this.selectedObjectType(),
  ...
  getTypeLabel(stat: DossierByEquipmentTypeChartStat): string {
