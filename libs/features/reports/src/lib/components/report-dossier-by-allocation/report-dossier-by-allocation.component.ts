...
import {
  ...
  InputUserLookupItem,
  DossierByAllocationFilter,
  DossierByAllocationChartStat,
  DossierByAllocationRatioStat
} from '../../data-access/report-dossier-by-allocation.service';
import { REPORT_STATISTICS_DOSSIER_LIST_CONFIGS, REPORT_STATISTICS_CREATOR_GRID_CONFIGS } from '../../data-access/report-statistics.config';
import {
  buildYearOptions,
  yearToFilterParam,
...
import { ReportStatisticsCreatorGridComponent } from '../report-statistics-creator-grid/report-statistics-creator-grid.component';
...
@Component({
  selector: 'app-report-dossier-by-allocation',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './report-dossier-by-allocation.component.html',
  styleUrls: ['./report-dossier-by-allocation.component.scss']
})
export class ReportDossierByAllocationComponent implements OnInit {
  private reportService = inject(ReportDossierByAllocationService);
  ...
  appliedFilter = signal<DossierByAllocationFilter>({
    unitId: null,
    objectType: 0,
  ...
  appliedObjectType = computed(() => this.appliedFilter().objectType ?? 0);

  appliedFilterParams = computed((): Record<string, string | number | null | undefined> => ({
    unitId: this.appliedFilter().unitId ?? null,
    objectType: this.appliedFilter().objectType ?? null,
    ...yearToFilterParam(this.appliedFilter().year ?? 0),
    createdBy: this.appliedFilter().createdBy ?? null
  }));
  ...
  inputUsers = signal<InputUserLookupItem[]>([]);
  ...
  selectedCreatedBy = signal<string | null>(null);

  activeTab = signal<MainTabMode>('stats');
  loading = signal<boolean>(false);
  exporting = signal<boolean>(false);

  chartStats = signal<DossierByAllocationChartStat[]>([]);
  ratioStats = signal<DossierByAllocationRatioStat[]>([]);
  ...
  private commitFilterFromSelection(): void {
    ...
    this.reportService.exportExcel(filter)
      ...
      .subscribe({
        next: (blob) => {
          ...
          a.href = url;
          a.download = `DanhSachHoSo_PhanBo_${buildExportYearSuffix(this.selectedYear())}_${new Date().getTime()}.xlsx`;
          a.click();
