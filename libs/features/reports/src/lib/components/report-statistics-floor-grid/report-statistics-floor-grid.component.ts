...
export class ReportStatisticsFloorGridComponent {
  private statisticsService = inject(ReportStatisticsService);

  gridConfig = input.required<ReportStatisticsFloorGridConfig>();
  filter = input<Record<string, string | number | string[] | null | undefined>>({});
  active = input(false);
  filterVersion = input(0);

  items = signal<ReportStatisticsFloorGridItem[]>([]);
  ...
  private loadData(): void {
    ...
    this.statisticsService
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('Lỗi tải lưới hồ sơ theo tầng lưu trữ:', err);
          this.items.set([]);
