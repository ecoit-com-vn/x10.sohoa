...
import {
  ReportDossierByManufactureYearService,
  DossierByManufactureYearFilter,
  DossierByManufactureYearChartStat
} from '../../data-access/report-dossier-by-manufacture-year.service';
import { UnitLookupItem } from '../../data-access/report-dossier-by-year.service';
import {
  REPORT_STATISTICS_DOSSIER_LIST_CONFIGS,
  REPORT_STATISTICS_EQUIPMENT_GRID_CONFIGS
} from '../../data-access/report-statistics.config';
import { buildYearOptions } from '../../data-access/report-year-filter.util';
import { ReportStatisticsDossierListComponent } from '../report-statistics-dossier-list/report-statistics-dossier-list.component';
import { ReportStatisticsEquipmentGridComponent } from '../report-statistics-equipment-grid/report-statistics-equipment-grid.component';
...
function manufactureYearToFilterParam(year: number): { manufactureYear?: number } {
  return year > 0 ? { manufactureYear: year } : {};
}

@Component({
  selector: 'app-report-dossier-by-manufacture-year',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
    ...
    ReportStatisticsEquipmentGridComponent
  ],
  providers: [MessageService],
  templateUrl: './report-dossier-by-manufacture-year.component.html',
  styleUrls: [
    '../report-dossier-by-year/report-dossier-by-year.component.scss',
    './report-dossier-by-manufacture-year.component.scss'
  ]
})
export class ReportDossierByManufactureYearComponent implements OnInit, AfterViewInit {
  private reportService = inject(ReportDossierByManufactureYearService);
  ...
  @ViewChild(ReportStatisticsEquipmentGridComponent)
  equipmentGrid?: ReportStatisticsEquipmentGridComponent;
  ...
  readonly dossierListConfig = REPORT_STATISTICS_DOSSIER_LIST_CONFIGS.DOSSIER_BY_MANUFACTURE_YEAR;
  readonly equipmentGridConfig = REPORT_STATISTICS_EQUIPMENT_GRID_CONFIGS.DOSSIER_BY_MANUFACTURE_YEAR;
  filterVersion = signal(0);

  reportFilter = computed(() => ({
    unitId: this.selectedUnitId(),
    stationIds: this.selectedStationIds().filter((id) => id?.trim()),
    ...manufactureYearToFilterParam(this.selectedManufactureYear())
  }));
  ...
  selectedManufactureYear = signal<number>(0);

  activeTab = signal<MainTabMode>('stats');
  loading = signal<boolean>(false);
  exporting = signal<boolean>(false);

  chartStats = signal<DossierByManufactureYearChartStat[]>([]);
  ...
  loadLookups(): void {
    ...
    this.reportService.getUnitsLookup().subscribe({
      next: (units) => {
        ...
        this.loadManufactureYearsLookup();
      },
      error: (err) => {
  ...
  private loadManufactureYearsLookup(): void {
    this.reportService.getManufactureYearsLookup().subscribe({
      next: (years) => {
        this.years.set(years || []);
        this.loadStatsData();
      },
      error: (err) => {
        console.error('Lỗi tải danh sách năm sản xuất:', err);
        this.years.set([]);
        this.loadStatsData();
      }
    });
  ...
  loadStatsData(): void {
    const filter: DossierByManufactureYearFilter = {
      unitId: this.selectedUnitId(),
    ...
    this.reportService.exportExcel(filter)
      ...
      .subscribe({
        next: (blob) => {
          ...
          a.href = url;
          a.download = `DanhSachHoSo_NamSanXuatThietBi_${new Date().getTime()}.xlsx`;
          a.click();
  ...
  getTypeLabel(stat: DossierByManufactureYearChartStat): string {
