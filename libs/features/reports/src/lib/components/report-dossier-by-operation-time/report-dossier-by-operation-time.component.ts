...
import {
  ReportDossierByOperationTimeService,
  DossierByOperationTimeFilter,
  DossierByOperationTimeSummaryStats
} from '../../data-access/report-dossier-by-operation-time.service';
...
@Component({
  selector: 'app-report-dossier-by-operation-time',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './report-dossier-by-operation-time.component.html',
  styleUrls: [
    '../report-dossier-by-year/report-dossier-by-year.component.scss',
    './report-dossier-by-operation-time.component.scss'
  ]
})
export class ReportDossierByOperationTimeComponent implements OnInit, AfterViewInit {
  private reportService = inject(ReportDossierByOperationTimeService);
  ...
  readonly dossierListConfig = REPORT_STATISTICS_DOSSIER_LIST_CONFIGS.DOSSIER_BY_OPERATION_TIME;
  readonly stationGridConfig = REPORT_STATISTICS_STATION_GRID_CONFIGS.DOSSIER_BY_OPERATION_TIME;
  ...
  summaryStats = signal<DossierByOperationTimeSummaryStats | null>(null);
  ...
  loadStatsData(): void {
    const filter: DossierByOperationTimeFilter = {
      unitId: this.selectedUnitId(),
      objectType: this.selectedObjectType(),
      fromDate: toDateOnlyParam(this.selectedFromDate()),
      toDate: toDateOnlyParam(this.selectedToDate())
    ...
    this.reportService.exportExcel(filter)
      ...
      .subscribe({
        next: (blob) => {
          ...
          a.href = url;
          a.download = `DanhSachHoSo_ThoiGianVanHanh_${new Date().getTime()}.xlsx`;
          a.click();
