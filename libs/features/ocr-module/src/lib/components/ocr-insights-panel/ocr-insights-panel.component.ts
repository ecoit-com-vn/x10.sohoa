...
/**
 * Điểm neo audit duy nhất cho 6 module Nhóm A (88, 90, 92, 93, 94, 95) trên tài liệu hồ sơ/thiết bị
 * đã số hóa sẵn — mở từ dossier-documents-tab / equipment-documents, KHÔNG đụng logic submit/re-extract
 * hiện có của 2 màn hình đó.
 */
@Component({
...
export class OcrInsightsPanelComponent implements OnChanges {
  @Input() visible = false;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['visible']) {
      if (this.visible && this.sourceDocument && !this.jobId()) {
        this.loadJob();
  ...
  classifyScriptType(): void {
    ...
    this.ocrModuleService.classifyScriptType(id).subscribe({
      next: (summary) => {
  ...
  runFormulaRecognition(): void {
    ...
    this.ocrModuleService.runFormulaRecognition(id).subscribe({
      next: (summary) => {
  ...
  runSealSignatureDetection(): void {
    ...
    this.ocrModuleService.runSealSignatureDetection(id).subscribe({
      next: (summary) => {
  ...
  runTemplateDiff(): void {
    ...
    this.ocrModuleService.runTemplateDiff(jobId, templateId).subscribe({
      next: (summary) => {
  ...
  runSpellcheck(): void {
    ...
    this.ocrModuleService.runSpellcheck(id).subscribe({
      next: (summary) => {
  ...
  runErrorAnalysis(): void {
    ...
    this.ocrModuleService.runErrorAnalysis(id).subscribe({
      next: (errors) => {
  ...
  private reset(): void {
    ...
    this.scriptTypeSummary.set(null);
    this.formulaSummary.set(null);
    this.sealSignatureSummary.set(null);
    this.templates.set([]);
    this.selectedTemplateId.set(null);
    this.templateDiffResults.set([]);
    this.newTemplateName.set('');
    this.spellcheckSummary.set(null);
    this.manualEditText.set({});
    this.errorAnalysisList.set([]);
  }
}
