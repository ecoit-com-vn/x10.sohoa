...
export class DossierDocumentsTabComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
   ...
   * Mở tab mới thay vì điều hướng tại chỗ để không mất dữ liệu đang nhập ở form Cập nhật hồ sơ.
   */
  onOpenOcrInsights(doc: DossierDocumentItem): void {
    if (!doc.latestVersionId) return;

    const basePath = this.kindId === 1 ? 'digitization/my-dossiers' : 'my-dossiers';
    const url =
      `/#/dossier-management/${basePath}/${this.dossierId}/documents/${doc.latestVersionId}/ocr-analysis` +
