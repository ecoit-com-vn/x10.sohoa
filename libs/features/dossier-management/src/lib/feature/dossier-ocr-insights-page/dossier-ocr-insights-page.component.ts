...
/**
 * Trang chi tiết "Phân tích OCR nâng cao" — thay cho popup cũ ở tab Tài liệu đính kèm,
 * mở trong tab mới để không mất dữ liệu đang nhập ở form Cập nhật hồ sơ.
 */
@Component({
  selector: 'app-dossier-ocr-insights-page',
  standalone: true,
  imports: [CommonModule, WfBreadcrumbComponent, OcrInsightsContentComponent],
  templateUrl: './dossier-ocr-insights-page.component.html',
  styleUrl: './dossier-ocr-insights-page.component.scss',
})
export class DossierOcrInsightsPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!dossierId || !documentVersionId) {
      this.loading.set(false);
      this.errorMessage.set('Thiếu thông tin tài liệu để phân tích OCR.');
      return;
    }

    this.documentService.getDigitizationProgress(dossierId, documentVersionId).subscribe({
      next: (progress) => {
        this.loading.set(false);
