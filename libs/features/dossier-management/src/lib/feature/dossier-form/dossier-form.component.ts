    ...
    <div class="wf-card" style="position: relative;">
      ...
      <div style="display: flex; flex-direction: column; gap: 16px; margin-bottom: 24px;">
        ...
        <div style="display: flex; gap: 16px; flex-wrap: wrap;">
          ...
          <div *ngIf="isEditMode() && !usePublishApi && !hideGridTypeField" class="form-group" style="flex: 1 1 240px; min-width: 220px;">
    ...
    <p-dialog
      ...
      <div style="display: flex; flex-direction: column; gap: 16px; padding: 8px 0 16px;">
        ...
        <div *ngIf="nextStepInfo()?.requiresNextAssignee" class="form-group">
          ...
          <div style="margin-top: 4px; font-size: 0.8rem; color: #64748b;" *ngIf="nextStepInfo()?.staticAssigneeId">Bước này có cấu hình người xử lý cụ thể — có thể chọn trong danh sách bên dưới.</div>
    ...
    <p-dialog [visible]="showFormActionDialog()" 
      ...
      <div style="display: flex; flex-direction: column; gap: 16px; padding: 8px 0 16px;">
        ...
        <div class="form-group" *ngIf="pendingActionBtn()?.requiresUser && !isRejectLabel(pendingActionBtn()?.label || '')" style="display: flex; flex-direction: column; gap: 6px;">
          ...
          <div style="margin-top: 4px; font-size: 0.8rem; color: #64748b;" *ngIf="pendingActionBtn()?.staticAssigneeId">Bước này có cấu hình người xử lý cụ thể — có thể chọn trong danh sách bên dưới.</div>
...
export class DossierFormComponent implements OnInit {
  ...
  private tryAutoGenerateDossierValues() {
    ...
    const unitId = firstInfra?.unitId ?? firstInfra?.UnitId;
    const unitCode = this.findOrganizationUnitCode(unitId);
    if (!codeField || !unitCode || !infraCode || !typeCode || !firstInfraId || !typeId) return;

    const requestId = ++this.autoCodeRequestId;
    this.service.getDossiers({
      menuScope: 'creator',
      kindId: this.kindIdSignal(),
      infrastructureId: String(firstInfraId),
      dossierTypeId: String(typeId),
      page: 1,
      pageSize: 1,
    }).subscribe({
      next: (res) => {
        if (requestId !== this.autoCodeRequestId) return;
        const total = Number(res?.totalCount ?? res?.TotalCount ?? res?.data?.totalCount ?? res?.Data?.TotalCount ?? 0);
        const sequence = Number.isFinite(total) ? total + 1 : 1;
        this.formData[codeField.key] = `${unitCode}.${infraCode}.${typeCode}.${String(sequence).padStart(3, '0')}`;
        this.refreshAutoGeneratedValues();
      },
      error: () => {
        if (requestId === this.autoCodeRequestId) {
          this.formData[codeField.key] = `${unitCode}.${infraCode}.${typeCode}.001`;
          this.refreshAutoGeneratedValues();
        }
      },
    });
  ...
  private findOrganizationUnitCode(unitId: unknown): string {
    if (unitId == null) return '';
    const findInTree = (items: any[]): any => {
      for (const item of items) {
        if (Number(item.id ?? item.Id) === Number(unitId)) return item;
        const children = item.children ?? item.Children ?? item.items ?? item.Items ?? [];
        const found = Array.isArray(children) ? findInTree(children) : null;
        if (found) return found;
      }
      return null;
    };

    const unit = findInTree(this.organizationUnits());
    return unit ? String(unit.code ?? unit.Code ?? '').trim() : '';
  }
